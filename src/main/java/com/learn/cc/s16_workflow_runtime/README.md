...
```bash
mvn -q compile exec:java -Dexec.mainClass=com.learn.cc.s16_workflow_runtime.AgentLoop
```

进入交互模式后，可以先让模型读取改动，再把内容放进 `args.changes` 并运行保存好的 `review-changes` workflow。

> 本 Java 版用 `MockRunner`（演示用，不实际调 LLM 做 sub-agent），也没有 Python 版的 `demo` / `resume` 子命令。journal / 续跑逻辑在 `WorkflowJournal` 里：交互中给 `Workflow` 工具传 `resume_from_run_id` 即命中缓存，全命中时显示 `agents=0 tokens=0`。
